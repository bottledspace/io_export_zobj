...
bl_info = {
    ...
    "version": (1,0,0),
    "blender": (2, 80, 0),
    "location": "File > Export > ZOBJ",
    "description": "Export ZOBJ models",
...
# 20ZZ Function:
def write_animations(scene, context, filepath):
...
# 20ZZ Function:
def write_armature(context, filepath):
...
def write_file(filepath, objects, depsgraph, context, scene,
               EXPORT_TRI=False,
               EXPORT_EDGES=False,
    ...
    animfilepath = os.path.splitext(filepath)[0] + ".anim"
    skelfilepath = os.path.splitext(filepath)[0] + ".skl"
    ...
    with ProgressReportSubstep(progress, 2, "OBJ Export path: %r" % filepath, "OBJ Export Finished") as subprogress1:
        ...
        if armsize != 0:
            if bpy.context.active_object.type == 'ARMATURE':
                write_armature(context, skelfilepath) # 20ZZ
        if animsize != 0:
            if bpy.context.active_object.type == 'ARMATURE':
...
def _write(context, filepath,
    ...
    with ProgressReport(context.window_manager) as progress:
        ...
        for frame in scene_frames:
            ...
            write_file(full_path, objects, depsgraph, context, scene,
                       EXPORT_TRI,
                       EXPORT_EDGES,
                       EXPORT_SMOOTH_GROUPS,
                       EXPORT_SMOOTH_GROUPS_BITFLAGS,
...
class ExportZOBJ(bpy.types.Operator, ImportHelper):
    bl_idname = "export_mesh.zobj"
    bl_label = "Export ZOBJ"
    bl_description = """Save ZOBJ mesh, material and animation data."""

    filename_ext = ".zobj"
    filter_glob: StringProperty(default="*.zobj", options={'HIDDEN'})
    
    keep_objex: BoolProperty(default=False, name="Keep OBJEX")
    use_links_hierarchy: BoolProperty(default=True, name="Link's Hierarchy")
    use_playas: BoolProperty(default=True, name="PlayAs")
    
    def execute(self, context):
        TEMP = os.getenv('TEMP')
        CWD = ntpath.dirname(ntpath.abspath(__file__))

        objexfilepath = ntpath.splitext(self.filepath)[0] + ".objex"
        if not self.keep_objex:
            objexfilepath = ntpath.join(TEMP, ntpath.basename(objexfilepath))
        print("TEMP=",TEMP,"objexfilepath=", objexfilepath)
        ...
        keywords = self.as_keywords(ignore=(
            ...
            "use_links_hierarchy",
            "use_playas"))
        keywords["global_matrix"] = Matrix.Scale(1.0, 4)
        keywords["filepath"] = objexfilepath
        save(context, **keywords)  # FIXME check the return!
    ...
    def draw(self, context):
        ...
        layout.prop(operator, 'keep_objex')
        layout.prop(operator, 'use_links_hierarchy')
        layout.prop(operator, 'use_playas')

def menu_export(self, context):
    self.layout.operator(ExportZOBJ.bl_idname, text="ZOBJ (.zobj)")

def register():
    bpy.utils.register_class(ExportZOBJ)
    bpy.types.TOPBAR_MT_file_export.append(menu_export)
  
def unregister():
    bpy.utils.unregister_class(ExportZOBJ)
