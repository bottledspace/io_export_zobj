...
if ob.type == 'ARMATURE':
     ...
     print(rig.name)
     for b in limbs[:]:
         parents = rig.bones[b.name].parent_recursive
         plen = len(parents)
         if plen != 0:
             #print(parents[plen-1])
             if parents[plen-1] == limbs[0]:
                 print("OK")
             else:
                 self.report({'WARNING'}, "All bones must be parented to bone 0!")
                 #print("Fail")
                 return {'FINISHED'}'''
        ...
        for mtl_mat_name, mat, face_img in mtl_dict_values:
            ...
            if face_img:  # We have an image on the face!
                ...
                if filepath:  # may be '' for generated images
                    ...
                    #20ZZ Clamp
                    if face_img.use_clamp_x:
                        fw('uprop c\n')
                    if face_img.use_clamp_y:
                        fw('vprop c\n')
                    #20ZZ TexGen
                    if face_img.mapping == 'REFLECTION':
                        fw('texgen on\n')
                    #20ZZ Scaling Factor
                    if face_img.display_aspect[0] > 1.0:
                        fw('scaleS %.6f\n' % (face_img.display_aspect[0] / 1000))
                    if face_img.display_aspect[1] > 1.0:
                        fw('scaleT %.6f\n' % (face_img.display_aspect[1] / 1000))
                    del filepath
                else:
...
# 20ZZ Function:
def write_animations(scene, context,
                   filepath
                   ):

    file = open(filepath, "w", encoding="utf8", newline="\n")

    file.write("anim_total %d\n"%(len(bpy.data.actions)))
    prev_action = bpy.context.active_object.animation_data.action
    prev_frame = scene.frame_current
    for anim in bpy.data.actions[:]:
        #print(anim.name)
        #print(" > range %d - %d"%(anim.frame_range[0],anim.frame_range[1]))
        bpy.context.active_object.animation_data.action = bpy.data.actions[anim.name]
        file.write("frames %d \"%s\"\n"%(int(anim.frame_range[1])-int(anim.frame_range[0]),anim.name))

        for frame in range(int(anim.frame_range[0]), int(anim.frame_range[1])):
            bpy.context.scene.frame_set(frame)
            root = bpy.context.active_object.pose.bones[0]
            file.write("l %f %f %f\n"%(root.location[0],root.location[1],root.location[2]))
            for bone in bpy.context.active_object.pose.bones:
                euler = bone.rotation_quaternion.to_euler('XYZ')
                #print("r %.03f %.03f %.03f %.03f"%(bone.rotation_quaternion[0],bone.rotation_quaternion[1],bone.rotation_quaternion[2],bone.rotation_quaternion[3]))
                file.write("r %.03f %.03f %.03f\n"%(euler[0],euler[1],euler[2]))
    # Restore things like scene number and action
    bpy.context.active_object.animation_data.action = prev_action
...
#            file.write("%f %f %f %f\n"%(bone.rotation_quaternion[0],bone.rotation_quaternion[1],bone.rotation_quaternion[2],bone.rotation_quaternion[3]))
...
#            #file.write("%sROOT %s\n" % (indent_str, bone_name))
#            file.write("%s+ \"%s\"\n" % (indent_str, bone_name))
        file.write("%s+ \"%s\" {%.6f %.6f %.6f}\n" % (indent_str, bone_name, loc.x * global_scale, loc.y * global_scale, loc.z * global_scale))
...
# context is a 20ZZ addition
def write_file(filepath, objects, scene, context,
               EXPORT_TRI=False,
               EXPORT_EDGES=False,
    ...
    with ProgressReportSubstep(progress, 2, "OBJ Export path: %r" % filepath, "OBJ Export Finished") as subprogress1:
        with open(filepath, "w", encoding="utf8", newline="\n") as f:
            ...
            sklfilepath = os.path.splitext(filepath)[0] + ".skl" # 20ZZ
            animfilepath = os.path.splitext(filepath)[0] + ".anim" # 20ZZ
            if EXPORT_MTL:
                mtlfilepath = os.path.splitext(filepath)[0] + ".mtl"
                # filepath can contain non utf8 chars, use repr
                armsize = len(bpy.data.armatures)
                animsize = len(bpy.data.actions)
                fw('mtllib %s\n' % repr(os.path.basename(mtlfilepath))[1:-1])
                if armsize != 0:
                    fw('skeleton %s\n' % repr(os.path.basename(sklfilepath))[1:-1]) # 20ZZ
                if animsize != 0:
                    fw('animations %s\n' % repr(os.path.basename(animfilepath))[1:-1]) # 20ZZ
            ...
            for i, ob_main in enumerate(objects):
                ...
                for ob, ob_mat in obs:
                    with ProgressReportSubstep(subprogress1, 6) as subprogress2:
                        ...
                        # 20ZZ code for group access...
                        fw('VERTGROUPORDER %d' % (len(ob.vertex_groups)) ) # Mark the start of the vertex groups in the .objex
                        for group in ob.vertex_groups: # List vertex groups...
                            fw(' "%s"' % (group.name ) )
                        fw('\n') # Decided to leave everything listed on one line, and this breaks the line afterwards
                        current_vertgroup=-1

                        # Vert
                        for v in me_verts:
                            heaviest_weight=0.0
                            heaviest_weight_vertgroup=-1
                            for group in v.groups: # Start 20ZZ test code for group/weight testing...
                                if group.weight > heaviest_weight:
                                    #fw('heavy %f, group %d...\n' % (group.weight,group.group) )
                                    heaviest_weight=group.weight
                                    heaviest_weight_vertgroup=group.group

                            if heaviest_weight_vertgroup != current_vertgroup:
                                fw('bone %d\n' % heaviest_weight_vertgroup)
                                current_vertgroup=heaviest_weight_vertgroup
                            fw('v %.6f %.6f %.6f\n' % v.co[:])
        ...
        if EXPORT_MTL:
            ...
            if armsize != 0:
                if bpy.context.active_object.type == 'ARMATURE':
                    write_armature(context, sklfilepath) # 20ZZ
            if animsize != 0:
                if bpy.context.active_object.type == 'ARMATURE':
                    write_animations(scene, context, animfilepath) # 20ZZ
...
def _write(context, filepath,
    ...
    with ProgressReport(context.window_manager) as progress:
        ...
        for frame in scene_frames:
            ...
            write_file(full_path, objects, scene, context, # 20ZZ context
                       EXPORT_TRI,
                       EXPORT_EDGES,
                       EXPORT_SMOOTH_GROUPS,
                       EXPORT_SMOOTH_GROUPS_BITFLAGS,
